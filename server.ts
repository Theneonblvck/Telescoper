...
// Caching
const cache = new NodeCache({ stdTTL: 86400 }); // Cache for 24 hours (86400 seconds)
...
app.post('/api/brave/search', async (req, res) => {
    ...
    if (!braveApiKey) return res.status(500).json({ error: 'Brave API not configured' });
...
app.post('/api/google/search', async (req, res) => {
    ...
    if (targetCseId) {
        ...
        if (!googleSearchApiKey) return res.status(500).json({ error: 'Google Search API Key missing' });
